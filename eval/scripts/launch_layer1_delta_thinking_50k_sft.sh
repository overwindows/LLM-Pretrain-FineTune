#!/bin/bash
# Launch script for Layer1_delta thinking-variant SFT on 50K curation data.
# Designed for a single 8-GPU A100 node (intended for node B in the
# 2-node parallel layout — node A runs layer0, node B runs layer1_delta).
#
# Use:
#   setsid nohup ./scripts/launch_layer1_delta_thinking_50k_sft.sh \
#       > logs/layer1_delta_thinking_50k_4o-v1.log 2>&1 < /dev/null &
...
export WANDB_TAGS=layer1_delta,sft,qwen3-4b-thinking,gpt4o-teacher,a100,50k,no-mask-think
...
accelerate launch \
    ...
    --config configs/sft/layer1_delta_thinking_50k_4o-v1.yaml
