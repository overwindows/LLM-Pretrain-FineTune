...
@dataclass
class ParseResult:
    """Output of :func:`parse_completion`.

    Attributes
    ----------
    parsed:
        Normalized Layer1 structure (a list of interest dicts) when both
        ``parse_ok`` and ``schema_ok`` are true; otherwise ``None``.
    parse_ok:
        ``True`` iff the post-think text was successfully parsed as JSON.
    schema_ok:
        ``True`` iff the parsed JSON satisfies the Layer1 schema.
    had_think_block:
        ``True`` iff a ``<think>...</think>`` block was found and stripped.
    errors:
        Human-readable error strings collected during parsing/validation.
    raw_post_think:
        The string fed to ``json.loads`` (post strip), useful for logging.
    """
...
def strip_think(text: str) -> tuple[str, bool]:
    ...
    Handles three cases:
    - Well-formed ``<think>...</think>`` → returns text after ``</think>``.
    - Closing ``</think>`` only (no opening tag, common in some templates)
      → returns text after the close tag.
    - No think markers → returns input unchanged.

    A truncated/unclosed ``<think>`` with no close tag is left as-is, which
    will likely fail JSON parsing later. That is the desired behaviour: such
    rollouts deserve a zero gate.
    """
    if not text:
        return "", False
...
def _normalize_interest(raw: Any, idx: int) -> tuple[dict | None, list[str]]:
    ...
    if not name and not topics_norm:
        # Interest is completely empty; drop it.
        return None, errs
...
def parse_completion(completion: str) -> ParseResult:
    ...
    # The Layer1 schema is a list at the top level. Some models wrap it as
    # ``{"interests": [...]}`` — accept that as a soft fallback.
