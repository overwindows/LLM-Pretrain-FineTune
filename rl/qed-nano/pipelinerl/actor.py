...
# Change: added _is_deterministic_bad_request() and made schedule_rollouts treat
# deterministic HTTP 400/413 (oversized/malformed prompt) as skip-this-rollout-group
# instead of retrying/stopping the actor. See ../../UPSTREAM.md.
import asyncio
import logging
import math
...
def _is_deterministic_bad_request(e: BaseException) -> bool:
    """Return True for client errors that will fail identically on every retry,
    e.g. an oversized / malformed prompt rejected by the inference server with
    HTTP 400 (Bad Request) or 413 (Payload Too Large).

    Such a rollout is deterministically doomed: retrying or re-queuing wastes
    work and, once attempts are exhausted, would otherwise crash the whole
    actor. The caller should instead drop the offending rollout group and keep
    the actor alive.
    """
    status = getattr(e, "status", None)
    if isinstance(e, aiohttp.ClientResponseError) and status in (400, 413):
        return True
    # Fallback: some clients surface the server's 400 body as a plain error
    # string rather than a ClientResponseError with a status code.
    msg = str(e).lower()
    return (
        "maximum context length" in msg
        or "longer than the maximum model length" in msg
        or "please reduce the length" in msg
    )
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    last_error = e
                    if _is_deterministic_bad_request(e):
                        # Deterministic client error (e.g. oversized prompt):
                        # retrying is pointless, propagate immediately so the
                        # group can be dropped without burning retries.
                        raise
            ...
            # Deterministic client errors (e.g. an oversized prompt rejected
            # with HTTP 400/413) will fail identically for every rollout in the
            # group. Drop the whole group and keep the actor alive instead of
            # retrying and eventually crashing the entire run.
            if _is_deterministic_bad_request(e):
                logger.warning(
                    f"Dropping group {group_id} due to deterministic bad request "
                    f"(HTTP {getattr(e, 'status', '?')}, likely oversized prompt); "
                    f"actor continues: {type(e).__name__}: {e}"
                )
                group_rollouts.pop(group_id, None)
                finished_rollouts += 1
            ...
            elif requeue_count < MAX_REQUEUE_ATTEMPTS:
                logger.warning(
