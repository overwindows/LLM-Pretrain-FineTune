...
# Change: GPU pool via PIPELINERL_GPUS env; actor vLLM base port via
# actor_llm_start_port; preprocessor-placement bug fixes (ZeroDivision / UnboundLocal).
# See ../../UPSTREAM.md.
import logging
import os
...
class WorldMap:
    def __init__(self, cfg: DictConfig, verbose: bool = False):
        ...
        # Preprocessor LLMs (use vllm_config). The preprocessor serves the KL
        # reference model via the same vllm_config as the actors, so size it from
        # vllm_config tp*pp. Falling back to the RC-actor value breaks when there
        # is a preprocessor but no RC actor (rc_actor_fraction==0 -> 0 -> div/0).
        preproc_llm_kwargs = self.cfg.get("preprocessor_vllm_config", self.cfg.vllm_config).vllm_kwargs
        preproc_tp = preproc_llm_kwargs.get("tensor-parallel-size", 1)
        preproc_pp = preproc_llm_kwargs.get("pipeline-parallel-size", 1)
        self.gpus_per_preprocessor_llm = preproc_tp * preproc_pp
        if cfg.world.get("preprocessor_fraction", 0) == 0:
            self.gpus_per_preprocessor_llm = 0
        ...
        # Explicit physical-GPU pool (single node): PIPELINERL_GPUS="1,2,3" lets us
        # avoid busy GPUs (e.g. GPU 0) without touching them. The listed ordinals are
        # the actual physical devices; they are handed to children verbatim as
        # CUDA_VISIBLE_DEVICES, so jobs land on exactly these GPUs.
        gpu_pool_env = os.environ.get("PIPELINERL_GPUS", "").strip()
        if self.world_size == 1 and gpu_pool_env:
            self.gpu_pool = [int(x) for x in gpu_pool_env.split(",") if x.strip() != ""]
            self.node_size = len(self.gpu_pool)
        else:
            self.gpu_pool = list(range(self.node_size))
        ...
        self.available_gpus = {i: set(self.gpu_pool) for i in reversed(range(self.world_size))}
    ...
    def _place_inference_jobs(self, cfg):
        ...
        # Place regular actor LLMs (port actor_llm_start_port+, default 8080)
        # Note: replica_idx must be offset by total_rc_actor_llms for NCCL group rank calculation
        actor_llm_start_port = cfg.world.get("actor_llm_start_port", 8080)
        for _ in range(cfg.world.replicas):
            for actor_llm_idx in range(self.llms_per_actor):
                ...
                local_idx = min(gpus)
                llm_url = f"http://{self.address_map[node]}:{actor_llm_start_port + local_idx}"
                logger.info(f"Placing actor LLM {actor_llm_idx} on node {node} at port {actor_llm_start_port + local_idx} with URL {llm_url}")
                self.add_job(
                    kind="actor_llm",
                    ...
                    node_rank=node,
                    gpus=gpus,
                    port=actor_llm_start_port + local_idx,
                    url=llm_url,
                )
